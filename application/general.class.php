...
Class general{
	...
	public function get_deposite_bank_list()
	{
		
	}
	public function sendtele($content)
	{
		global $db;
		$db->query("SELECT * FROM hicrm_users WHERE user_group = '1' OR user_group = '2' ");
		$admins = $db->fetch_object();
		foreach($admins as $ad)
		{
			if($ad->user_telegramid != "")
			{
				$path = "https://api.telegram.org/bot5274249150:AAEdPnQ7meSu24OJxurzc_m5GzakCk5nHbg";
				file_get_contents($path."/sendmessage?parse_mode=html&chat_id=".$ad->user_telegramid."&text=".$content);
			}
		}
	...
	public function captchar()
	{
		...
		$img_width = 120;
		$img_height = 40;
		$font = './uploads/monofont.ttf';
		header('Content-type: image/jpeg');
		 
		$image = imagecreate($img_width, $img_height); // create background image with dimensions
		imagecolorallocate($image, 228, 204, 255); // set background color
