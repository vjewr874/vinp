...
Class apiController extends baseController
{
	...
	{
		global $db;
		$sql = "SELECT * FROM game_datas WHERE game_time >= '".date("Y-m-d H:i:s")."' AND game_time < '".date("Y-m-d H:i:s",strtotime("+1 seconds"))."' ORDER BY id DESC LIMIT 1";
		$db->query($sql);
		$r = $db->fetch_object(true);
		$result = array();
		$result["nowtime"] = date("m/d/Y H:i:s");
		...
		$result["time"] = date("m/d/Y H:i:s",strtotime($r->game_time));
		$result["price"] = $r->game_data;
		$result["draw_id"] = $r->draw_id;
		$result["status"] = 200;
		echo json_encode($result);
	}
	public function uploadcmndadmin()
	{
		...
		if(isset($_FILES['file']))
		{
			...
			if(empty($errors)==true){
				...
				if($_POST['type'] == 1)
				{
					$db->query("UPDATE portal_customer_banks SET bank_cmnd_front = '".$FinalFilenameFront."' WHERE cid = '".$_POST['cid']."'");
				}
				elseif($_POST['type'] == 2)
				{
					$db->query("UPDATE portal_customer_banks SET bank_cmnd_back = '".$FinalFilenameFront."' WHERE cid = '".$_POST['cid']."'");
				}
				$result["status"] = 200;
	...
	public function banklinked()
	{
		...
		$cmndfrontfile = "";
		$cmndbackfile = "";
		if(isset($_FILES['cmndfront']) && isset($_FILES['cmndback']))
		{
			if(isset($_FILES['cmndfront']))
			{
				$errors= array();
				$file_name = $_FILES['cmndfront']['name'];
				$file_size =$_FILES['cmndfront']['size'];
				$file_tmp =$_FILES['cmndfront']['tmp_name'];
				$file_type=$_FILES['cmndfront']['type'];
				$file_ext=strtolower(end(explode('.',$_FILES['cmndfront']['name'])));
				$OriginalFilename = $FinalFilename = preg_replace('`[^a-z0-9-_.]`i','',$_FILES['cmndfront']['name']); 
				$cmndfrontfile = md5(time())."-".$FinalFilename;
				$expensions = array("jpg","png");
				if(in_array($file_ext,$expensions)=== false)
				{
					$message .= "Định dạng mặt trước CMND không được hỗ trợ, vui lòng chỉ sử dụng file ảnh. ";
					$error = true;
				}
				elseif($file_size > 5242880){
					$message .= 'Kích thước ảnh mặt trước CMND tối đa 5MB. ';
					$error = true;
				}
				else
				{
					move_uploaded_file($file_tmp,"./uploads/users/".$cmndfrontfile);
					$result["url_front"] = XC_URL."/uploads/users/".$cmndfrontfile;
				}
			}
			if(isset($_FILES['cmndback']))
			{
				$errors= array();
				$file_name = $_FILES['cmndback']['name'];
				$file_size =$_FILES['cmndback']['size'];
				$file_tmp =$_FILES['cmndback']['tmp_name'];
				$file_type=$_FILES['cmndback']['type'];
				$file_ext=strtolower(end(explode('.',$_FILES['cmndback']['name'])));
				$OriginalFilename = $FinalFilename = preg_replace('`[^a-z0-9-_.]`i','',$_FILES['cmndback']['name']); 
				$cmndbackfile = md5(time())."-".$FinalFilename;
				if(in_array($file_ext,$expensions)=== false)
				{
					$message .= "Định dạng mặt sau CMND không được hỗ trợ, vui lòng chỉ sử dụng file ảnh. ";
					$error = true;
				}
				elseif($file_size > 5242880){
					$message .= 'Kích thước ảnh mặt sau CMND tối đa 5MB. ';
					$error = true;
				}
				else
				{
					move_uploaded_file($file_tmp,"./uploads/users/".$cmndbackfile);
					$result["url_back"] = XC_URL."/uploads/users/".$cmndbackfile;
				}
			}
			if($error == false)
			{
				...
				$db->query("DELETE FROM portal_customer_banks WHERE cid = '".$_SESSION['user']['id']."'");
				$db->query("INSERT INTO portal_customer_banks(cid,bank_id,bank_account,bank_branch,bank_status) VALUES('".$_SESSION['user']['id']."','".$bank."','".$account."','".$branch."',1,'".$cmndfrontfile."','".$cmndbackfile."')");
				//$db->query("INSERT INTO portal_customer_banks() VALUES()");
				$result["status"] = 200;
			}
			else
			{
		...
		{
			$result["status"] = 500;
			$result["message"] = "Vui lòng tải lên ảnh mặt trước và mặt sau CMND";
		}
	...
	public function updateprofile()
	{
		...
		$db->query("UPDATE portal_customers SET cus_fullname = '".$fullname."', cus_address = '".$address."', cus_update_flag = 1 WHERE id = '".$_SESSION['user']['id']."' ORDER BY id DESC LIMIT 1");
		$result = array();
		$result["status"] = 200;
		echo json_encode($result);
	}
	public function uploadcmnd()
	{
		global $db;
		$result = array();
		...
		if(isset($_FILES['file']))
		{
			...
			if(empty($errors)==true){
				...
				if($_POST['type'] == "front")
				{
					$db->query("UPDATE portal_customer_banks SET bank_cmnd_front = '".$FinalFilenameFront."' WHERE cid = '".$_SESSION['user']['id']."'");
				}
				else
				{
					$db->query("UPDATE portal_customer_banks SET bank_cmnd_back = '".$FinalFilenameFront."' WHERE cid = '".$_SESSION['user']['id']."'");
				}
			}else
			{
				$result["status"] = 500;
		...
		*/
		...
		foreach($betlist as $bet)
		{
			$pt = array();
				...
				if($bet->pattern_1)
				{
					($bet->pattern_1 == 1)? array_push($pt,"Lên") : array_push($pt,"Xuống");
					$countbet++;
				}
				if($bet->pattern_2)
				{
				...
				$oods = ($countbet > 1)? "0.85/0.85": "0.85";
				$pattern = implode("/",$pt);
				$payout = '';
				if($bet->bet_status == 0)
				{
	...
	public function requestwithdraw()
	{
		...
		else
		{
			...
			$db->query("SELECT * FROM portal_customers WHERE id = '".$uid."' ORDER BY id DESC LIMIT 1");
			$user = $db->fetch_object(true);
			if($amount < $this->config->_config("minimun_withdraw"))
			{
				$result["status"] = 503;
				$result["message"] = "Số tiền rút tối thiểu là: ".number_format($this->config->_config("minimun_withdraw"),0)." VNĐ";
			}
			elseif($user->cus_balance >= $amount)
			{
				$fee = $this->config->_config("withdraw_fee")/100;
				$uid = $user->id;
				$total = $amount + $amount*$fee;
				$db->query("SELECT * FROM portal_customer_banks as b 
				LEFT JOIN system_banks as sb ON b.bank_id = sb.id
				WHERE b.cid = '".$uid."' ORDER BY b.id DESC LIMIT 1");
				$bankdata = $db->fetch_object(true);
				$bank = $bankdata->id;
				$db->query("UPDATE portal_customers SET cus_balance = cus_balance - ".$amount." WHERE id = '".$user->id."'");
				$db->query("INSERT INTO portal_withdraws(wd_code,wd_uid,wd_bank,wd_account,wd_branch,wd_amount,wd_status) VALUES('".$transcode."','".$uid."','".$bankdata->bank_code."','".$bankdata->bank_account."','".$bankdata->bank_branch."','".$amount."','0')");
				$db->query("INSERT INTO portal_transactions(uid,trans_code,trans_type,trans_bank,trans_method,trans_amount, trans_fee,trans_hash,trans_status,trans_note) VALUES('".$uid."','".$transcode."','2','".$bank."','1','".$amount."','".($amount*$fee)."','".bin2hex(mcrypt_create_iv(16, MCRYPT_DEV_URANDOM))."','1','Rút tiền từ tài khoản chính')");
				$result["message"] = "Giao dịch rút tiền của Quý Khách đã được xác nhận. Trung tâm đối soát thanh toán của Invest Pro Co., Ltd sẽ thực hiện kiểm tra hợp lệ và chuyển tiền cho Quý Khách trong thời gian sớm nhất. Xin lưu ý, quá trình này có thể cần từ 05 đến 10 phút làm việc! Xin cảm ơn!";
				$result["status"] = 200;
				//$db->query("SELECT * FROM portal_customers WHERE id = '".$uid."'");
				//$customer = $db->fetch_object(true);
				$content = "Khách hàng: <b>".$user->cus_fullname." (".$user->cus_username.")</b> vừa thực hiện yêu cầu rút tiền số: <b>".$transcode."</b> với số tiền: <b>".number_format($amount,0)." VND</b>.%0a[Invest Pro]";
				$this->helper->sendtele($content);
			}
			else
			{
				$result["status"] = 503;
	...
	public function deposite()
	{
		...
		else
		{
			...
			if($amount > 350000)
			{
				...
				$transdata = $this->config->_config("deposite_prefix").$transcode;
				$method = $_POST['method'];
				...
				$db->query("SELECT * FROM portal_customers WHERE id = '".$uid."'");
				$customer = $db->fetch_object(true);
				$content = "Khách hàng: <b>".$customer->cus_fullname." (".$customer->cus_username.")</b> vừa thực hiện yêu cầu nạp tiền số: <b>".$transdata."</b> với số tiền: <b>".number_format($amount,0)." VND</b>.%0a[Invest Pro]";
				$this->helper->sendtele($content);
				$result["status"] = 200;
			}
			else
	...
	public function register()
	{
		...
		$password = md5(mysql_real_escape_string($_POST['password']));
		$captchar = mysql_real_escape_string($_POST['captchar']);
		if($captchar == $_SESSION['captcha_num'])
		{
			$ref = 0;
			$db->query("SELECT * FROM system_referals WHERE referal_sub = '".$_SESSION['sub']."' ORDER BY referal_id DESC LIMIT 1");
			if($db->num_row())
			{
				$ref = $db->fetch_object(true)->referal_id;
			}
			...
			else
			{
				...
				$db->query("INSERT INTO portal_customers(cus_code,cus_username,cus_password,cus_fullname,cus_phone,cus_registed_date,cus_referal) VALUES('".$code."','".$username."','".$password."','".$name."','".$phone."','".date("Y-m-d H:i:s")."','".$ref."')");
	...
	public function login()
	{
		...
		elseif($member_login == 400)
		{
			$result["status"] = 400;
			$result["message"] = "Tên đăng nhập ngưng sử dụng";
		}
		else
		{
			$result["status"] = "500";
